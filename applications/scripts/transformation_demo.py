...
def transform_to_pose(matrix):
    pose = Pose()
    # TODO: fill this out
    res = np.matmul(matrix, [0, 0, 0, 1])
    pose.position.x = res[0]
    pose.position.y = res[1]
    pose.position.z = res[2]
    res = tft.quaternion_from_matrix(matrix)
    pose.orientation.x = res[0]
    pose.orientation.y = res[1]
    pose.orientation.z = res[2]
    pose.orientation.w = res[3]
    return pose
